...
def plot_obs_temp():
    for en in ens_list[model]:
        ...
        if en == ens:
            axs[1,m].plot((data_nh['anomaly']+data_sh['anomaly'])*0.5,linewidth=2,
...
def plot_temp():
    ...
    axs[1,m].fill_between(yearlist, np.percentile(post_temp_glob.values,5, axis=0),
                        np.percentile(post_temp_glob.values, 95, axis=0), color='red', alpha=0.3,label='90 C.I.')
...
def plot_ohc_obs():
    for en in ens_list[model]:
        ...
        if en == ens:
            axs[2,m].plot(cmip_0_700+cmip_700_bot,linewidth = 2,linestyle='-',color='black', label='"obs."')
        elif en == 2:
...
def plot_ohc():
    ...
    axs[2,m].fill_between(yearlist, np.percentile(post_OHCtot.values,5, axis=0),
                        np.percentile(post_OHCtot.values, 95, axis=0), color='red', alpha=0.3,label='90 C.I.')
...
def plot_post_erf():
    ...
    axs[0,m].plot(post_rf['Median'],color='black',label='Posterior mean')
...
model_org = { 'HadGEM3':'HadGEM3-GC31-LL'}
...
plt.savefig('Figures_ens/combined_results_'+model + 'selected_all.png')
